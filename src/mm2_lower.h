...
/* Lower raw MM2 interface forms into inert internal IR before ordinary HE
...
/* MM2 files are raw interface syntax, not HE scripts. */
bool cetta_mm2_atoms_have_top_level_eval(Atom **atoms, int n);

/* Raise one raw or lowered MM2 atom back to executable MM2 interface syntax.
   This is the inverse bridge used by the MORK runtime seam: lowered IR and
   already-interface MM2 terms both round-trip through one honest serializer. */
Atom *cetta_mm2_raise_atom(Arena *a, Atom *atom);

/* Render one raw or lowered MM2 atom as interface MM2 S-expression text. */
...
/* Encode one raw or lowered MM2 atom as stable MORK bridge expr bytes.
   This keeps the CeTTa<->MORK mutation boundary below UTF-8 interface text while
