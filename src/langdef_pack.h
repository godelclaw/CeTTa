 ...
 * what a future --compile-langdef tool would emit (rule IDs, profile interface,
 ...
 * (one observable interface rewrite), not the fine interpreter state machine.
...
typedef struct {
    ...
    const char *profiles;   /* authored profile interface for this rule */
