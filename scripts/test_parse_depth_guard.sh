...
for interface in parse parse-first; do
    printf '!(%s "%s")\n' "$interface" "$deep" > "$input"

    if ! "$bin" --profile he-extended --lang he "$input" > "$stdout" 2> "$stderr"; then
        cat "$stderr" >&2
        echo "FAIL: $interface depth guard process exited non-zero" >&2
        exit 1
    fi

    if grep -Eiq 'stack overflow|StackOverflow' "$stdout" "$stderr"; then
        cat "$stderr" >&2
        echo "FAIL: $interface depth guard reached process stack guard" >&2
        exit 1
    fi

    if ! grep -Fq 'ParseFailed' "$stdout"; then
        echo "FAIL: deeply nested $interface did not fail cleanly" >&2
